...
\left(
 \varepsilon ^{-b}h^c,
\eps^a
\right)\leq \varepsilon_{\rm thresh}^a =h^{\frac{ac}{a+b}  }$, or 
...
\begin{equation}\label{init_1D}
 \| u_j^{\eps}(0)-\eps v_j(0)\|_{L^2(\mathbb{R})} \leq C_0 h\eps \|p_0\|_{H^2(\mathbb{R})}.
\end{equation}


We denote by $\V^{\eps}(t,x)=\left(p^{\eps}(x,t),
u^{\eps}(x,t)\right)$ the solution of the hyperbolic heat equations $P^\eps$ and by
$\V^{\eps}_h(t,x)=
\left( \sum_{j\in \mathbb Z} p_j^{\eps}(t) {1}_{\Delta x_j}(x) ,  \sum_{j\in \mathbb Z} u_j^{\eps}(t) {1}_{\Delta x_j}(x) \right)$ the solution of the JL-(b) scheme $P_h^\eps$, in which ${1}_{\Delta x_j}(x)=1$ if $x\in\Delta x_j$ and $0$ elsewhere. We  reconstruct
...
$P^0$ (\ref{difffff})-(\ref{eq:diff-v}),
 and  $\W_h^\eps(t,x)= \left( \sum_{j\in \mathbb Z} p_j(t) {1}_{\Delta x_j}(x) ,
\varepsilon \sum_{j\in \mathbb Z} v_j(t)  {1}_{\Delta x_j}(x) \right)$ which is
the
...
\end{equation}
Finally the last term in (\ref{eq:eqn2}) can be bounded using the Jensen inequality 
$$
...
\|\V_h^{\eps}-\W^\eps_h\|_{L^2([0,T]\times\mathbb R)} \leq  \eps e^{T/2} \bigg(  C_0h + \sqrt{ \frac{1}{\sigma C_{\mathcal{M}} } \big( \frac12 + \frac{1}{\sigma^2} \big) } \bigg)  \|p_0\|_{H^2(\mathbb{R})},
\end{equation}
...
$
\|\V_h^{\eps}-\W^\eps_h\|_{L^2([0,T]\times\mathbb{R})} \leq \eps e^{T/2} \big(  C_0h + \sqrt{ \frac{1}{\sigma C_{\mathcal{M}} } ( \frac12 + \frac{1}{\sigma^2} )}  \big) \|p_0\|_{H^2(\mathbb{R})} $.  
The proof is ended.
\end{proof} 
...
\begin{equation*}
\|\V^{\eps}-\V^{\eps}_h \|_{L^2([0,T]\times \mathbb{R})}
\leq C \min\bigg( \sqrt \frac{h}\eps  \: , \: \eps + h + \eps \bigg)
\end{equation*}
where
\begin{equation*}
\begin{aligned}
C =  2 \max \bigg\{  & \sqrt{h\eps + 2T + \frac{\eps}{2} + \frac{\eps}{4C_{\mathcal{M}}} + \frac{\sigma}{4} }
\left\| \V^{\eps}_0
\right\|_{H^1(\mathbb{R})} \: ,  \\
&\|p_0\|_{H^3(\mathbb{R})} \max\bigg( e^{T/2} \bigg(  C_0h + \sqrt{ \frac{1}{\sigma C_{\mathcal{M}} } \big( \frac12 + \frac{1}{\sigma^2} \big) }  \bigg) \: ,\\
& \hspace{3cm} e^{T/2\sigma} + 
\eps \sqrt{\frac12 + \frac{4}{\mathcal{C}_M}\bigg( 3 + \frac{T}{\sigma}e^{T/\sigma} \bigg) }  \: ,  \frac{T}{\sigma^2}  \bigg) \bigg\} .
\end{aligned}
\end{equation*}
Using the general
method described at the beginning of this work in proposition \ref{prop:1}, 
...
\end{equation*}
By definition  there exists  a constant $C_{\mathcal{M}}>0$ such that 
\begin{equation} \label{eq:d7}
\left\{
\begin{aligned}
&l_{jr}\leq C_{\mathcal{M}}h , \qquad \forall j,r , \\
&|\Omega_j| \leq C_{\mathcal{M}}^2 h^2 , \qquad \forall j .
\end{aligned}
\right. 
\end{equation}
...
\item  The matrix $B_r$ is positive and there exists a
...
\begin{itemize}
\item $\mathbf V_h^\eps(t,\x)=\left(  \sum_{j\in Cells} p^\eps_j(t) 1_{\Omega_j}(\x) , \sum_{j\in Cells} \uu^\eps_j(t)  1_{\Omega_j}(\x) \right) $ is the solution of $\mathbf P_h^\eps$.
\item 
$\mathbf V^\eps(t,\x) =\left(p^\eps, \uu^\eps  \right)(t,\x)$ is the solution of $\mathbf P^\eps$,
\item $\mathbf W_h^\eps(t,\x) =\left(  \sum_{j\in Cells}  p^\eps_j(t) 1_{\Omega_j}(\x) ,  \sum_{j\in Cells}  \uu^\eps_j(t)  1_{\Omega_j} (\x) \right) $ is the solution of $\mathbf{DA}_h^\eps$.
Notice that an abuse of notations is been made with the solution
of $\mathbf P_h^\eps$.
\item $\mathbf W^\eps (t,\x) = \left( p ,-\frac\eps\sigma \nabla p  \right) (t,\x)$ is the solution of $\mathbf P^0$.
\end{itemize}
...
\left\{
\begin{aligned}
&\| \mathbf{V}_0 - \mathbf{V}^{\eps}_h(0)\|_{L^2(\Omega)} \leq C_0 h \|  \mathbf V ^\eps_0 \|_  { H^1(\Omega)  } ,\\
&\| \mathbf{W}_0 - \mathbf{W}^{\eps}_h(0)\|_{L^2(\Omega)} \leq C_0 h \|  \mathbf W ^\eps_0 \|_  { H^1(\Omega)  } .
\end{aligned}
\right.
\end{equation}
...
\begin{equation*}
\begin{aligned}
C = \|\mathbf{V}_0^\eps\|_{H^3(\Omega)}  \sqrt{2+T+\sigma } \: \hbox{max}\bigg\{ & \:  \frac{ C_{\mathcal{M}}^2 }{2}\sqrt{\frac{P}{C_1}} \bigg( \frac1{\alpha} + PC_{\delta}^2 \bigg) \: , \: \frac{C_{\mathcal{M}}^3P}{2C_1}  , \\
&  C_{\mathcal{M}} C_{\delta} P \bigg( \frac{1+C_0^2h^2}{C_1} + C_{\delta}  + h \bigg) \: , \: C_0^2 + C_{\mathcal{M}}^2  P C_{\delta}\bigg( 1 + \frac{2C_{\mathcal{M}}^2P}{C_1\alpha} \bigg) \: \bigg\}^{\frac12} .
\end{aligned}
\end{equation*}
The constants $P$, $C_{\delta}$, $C_{\mathcal{M}}$, $\alpha$, $C_0$ and $C_1$ are defined respectively in \eqref{nbmr}, (\ref{const_delta1}-\ref{const_delta2}), \eqref{eq:d7}, \eqref{geom3}, \eqref{constant_init} and \eqref{eq:d5.2}.
\end{proposition}
...
_r^{\eps})^2 
+ T \frac {P C_{\delta}^2 C_{\mathcal{M}} h }{2\gamma \eps} \vert \vert \mathbf{V}_0^{\eps} \vert\vert^2_{H^1(\Omega)}  +  \bigg( \frac1{\alpha} + PC_{\delta}^2 \bigg) \frac{ h C_{\mathcal{M}}^2 }{2}\sqrt{\frac{P}{C_1}}  \| \mathbf{V}_0^{\eps} \|_{H^1(\Omega)}^2,
\end{equation}
where the constants $P$, $C_{\delta}$, $C_{\mathcal{M}}$ and $C_1$ are defined respectively in \eqref{nbmr}, (\ref{const_delta1}-\ref{const_delta2}), \eqref{eq:d7} and \eqref{eq:d5.2}.
\end{pro}
\begin{proof}
...
\end{aligned}
\end{equation*}
We now look at the last term of this inequality, which we call $W$.
...
constant $C_{\mathcal{M}}>0$ such that $|\xr -\xj|\leq C_{\mathcal{M}} h$ and $\ljr \leq C_{\mathcal{M}} h$. Since $| \njr\otimes(\xr -\xj) \textbf{u}_r^{\eps} |^2 = | (\xr -\xj) ,  \textbf{u}_r^{\eps} |^2$, one obtains by using successively assumptions \eqref{eq:d5.2} and \eqref{nbmr}
\begin{equation*}\label{esti_beta}
\vert W \vert
\leq  \frac{\sigma h C_{\mathcal{M}}^2 }{\eps^2}\sqrt{\frac{P}{C_1}}
\bigg( \sum_r |V_r| \big( \textbf{u}_r^{\eps},\textbf{u}_r^{\eps}\big)\bigg)^{\frac12} \bigg(\sum_j \sum_r    | \delta_{j,r}(\uu^{\eps})|^2 \bigg)^{\frac12 }.
\end{equation*}
and therefore
\begin{equation}\label{eq:bound2}
\vert W \vert
\leq \frac{\sigma h C_{\mathcal{M}}^2 }{2\eps^2}\sqrt{\frac{P}{C_1}}
...
\delta_{j,r}(\uu^{\eps}) \bigg)^2 + \frac{\sigma h C_{\mathcal{M}}^2 }{2\eps^2}\sqrt{\frac{P}{C_1}} 
\bigg( \sum_r |V_r|  \big( \textbf{u}_r^{\eps},\textbf{u}_r^{\eps}\big)  +\sum_j \sum_r    | \delta_{j,r}(\uu^{\eps})|^2 \bigg)\\
\end{aligned}
\end{equation}
Furthermore, using the  classical interpolation results of proposition \ref{pro:interp} and once again the assumption  \eqref{nbmr}, one has 
\[
\sum_j \sum_r | \delta_{j,r}\uu^{\eps}|^2 \leq P C_{\delta}^2 \vert \vert \uu^\eps \vert\vert^2_{H^1(\Omega)}
.
\]
and
\[
\sum_j \sum_r l_{j,r}  \delta_{j,r} ( p^{\eps})^2 \leq P C_{\delta}^2 C_{\mathcal{M}} h \vert \vert p^\eps \vert\vert^2_{H^1(\Omega)}
.
\]
So we obtain
...
\sum_{j,r}l_{j,r}(\textbf{n}_{j,r},\textbf{u}_j^{\eps}-\textbf{u}
_r^{\eps})^2 dt  +  \frac {P C_{\delta}^2 C_{\mathcal{M}} h }{2\gamma \eps}\bigg(\vert \vert \uu^\eps \vert\vert^2_{L^2([0,T]; H^1(\Omega) )} +\vert \vert p^\eps \vert\vert^2_{L^2([0,T]; H^1(\Omega) )}  \bigg) \\
&+\frac{\sigma h C_{\mathcal{M}}^2 }{2\eps^2}\sqrt{\frac{P}{C_1}}  \| \textbf{u}_r^{\eps} \|_{L^2([0,T]\times\Omega)}^2 + \frac{\sigma h C_{\mathcal{M}}^2  C_{\delta}^2 P^{3/2} }{2\sqrt{C_1} \eps^2}  \vert \vert \uu^\eps \vert\vert^2_{L^2([0,T]; H^1(\Omega) )} .
\end{aligned}
\end{equation*}
...
\int_0^T E_2(t) dt \leq  \frac{hT}{\eps} C_{\mathcal{M}} C_{\delta} P \bigg( \frac{1+C_0^2h^2}{C_1} + h \bigg)   \|  \mathbf V ^\eps_0 \|^2_  { H^3(\Omega)  } ,
\end{equation} 
where the constants $P$, $C_{\delta}$, $C_{\mathcal{M}}$ and $C_0$ are defined respectively in \eqref{nbmr}, (\ref{const_delta1}-\ref{const_delta2}), \eqref{eq:d7} and \eqref{constant_init}.

\end{pro}
\begin{proof}
$E_2=A+B$ is made of two contributions.
Making use of the second set of inequalities
of the proposition \ref{pro:interp} and the assumptions \eqref{eq:d7} and \eqref{nbmr}, one gets concerning the first one
$$
| A|= \left |
\frac{1}{\eps}\sum_j \sum_r  |\Gamma_{j,r} | p_j^{\eps} (\mathbf{n}_{j,r} ,
...
\right |
\leq 
\frac{ C_{\mathcal{M}} C_{\delta} P }{\eps}  \sum_j h^{5/2} | p_j  ^\eps |
 \|  \mathbf V ^\eps (t) \|_  { H^3(\Omega_j)  }.
$$
And using the inequality $ab\leq \frac{1}{2}(a^2+b^2)$ 
$$
| A|\leq 
\frac{ C_{\mathcal{M}} C_{\delta} P }{ 2\eps}  \sum_j h^{3} | p_j  ^\eps |^2 + \frac{C_{\mathcal{M}} C_{\delta} P}{2 \eps} \sum_j h^{2} 
 \|  \mathbf V ^\eps (t) \|^2_  { H^3(\Omega_j)  }
$$
The assumption \eqref{eq:d5} yields
$$
| A| \leq 
\frac{hC_{\mathcal{M}} C_{\delta} P}{2C_1 \eps} \|  \mathbf V_h ^\eps (t) \|^2_  { L^2(\Omega)  } + \frac{C_{\mathcal{M}} C_{\delta} P h^2}{2 \eps}  
 \|  \mathbf V ^\eps (t) \|^2_  { H^3(\Omega)  }.
$$
The $L^2$ stability  (\ref{gggg})  of the scheme $\mathbf P_h ^\eps$ yields
that $
\|  \mathbf V_h ^\eps (t) \|^2_  { L^2(\Omega)  }\leq
\|  \mathbf V_h ^\eps (0) \|^2_  { L^2(\Omega)  }\leq\|  \mathbf V ^\eps_0 \|^2_  { L^2(\Omega)  }+C_0^2h^2 \|  \mathbf V ^\eps_0 \|^2_  { H^1(\Omega)  } $, where the last inequality comes from the initialization stage \eqref{constant_init}. With  the basic energy estimate (\ref{bee1}), and since $h$ is bounded, we obtain
$$
\int_0^T\vert A \vert dt \leq \frac{hT}{\eps} \frac{C_{\mathcal{M}} C_{\delta} P }{2 }\bigg( \frac{1+C_0^2h^2}{C_1} + h \bigg)   \|  \mathbf V ^\eps_0 \|^2_  { H^3(\Omega)  } .
$$
...
$$
\int_0^T\vert B \vert dt \leq \frac{hT}{\eps} \frac{C_{\mathcal{M}} C_{\delta} P }{2 }\bigg( \frac{1+C_0^2h^2}{C_1} + h \bigg)   \|  \mathbf V ^\eps_0 \|^2_  { H^3(\Omega)  },
$$
...
For all $\widehat{\gamma}>0$, the following bound holds
...
+
C_{\mathcal{M}}^2  P \bigg\{ C_{\delta}\bigg( 1 + \frac{2C_{\mathcal{M}}^2P}{C_1\alpha} \bigg) h^2 
+
\frac{ C_{\mathcal{M}} h}{2 C_1 \widehat{\gamma}\eps} \bigg\} \|  \mathbf{V}_0^{\eps} \|_{H^1(\Omega)}^2,
\end{aligned}
\end{equation}
where the constants $P$, $C_{\delta}$, $C_{\mathcal{M}}$ and $C_1$ are defined respectively in \eqref{nbmr}, (\ref{const_delta1}-\ref{const_delta2}), \eqref{eq:d7} and \eqref{eq:d5.2}.
...
by a Young's inequality with a convenient constant $C=\frac{C_1\alpha}{2C_{\mathcal{M}}^2P}$
...
S_1
\leq C_{\mathcal{M}}^2 \bigg( 1 + \frac{2C_{\mathcal{M}}^2P}{C_1\alpha} \bigg) \frac{h^2\sigma }{\eps^2}\sum_r \sum_j  \left|  \delta_{j,r}(\textbf{u}^{\eps}) \right|^2.
\end{equation*}
Using the standard interpolation result stressed in proposition \ref{pro:interp},
one has in dimension two
$| \delta_{j,r}(\textbf{u}^{\eps}) |
 \leq C_{\delta}  \|\uu^\eps(t)\|_{H^1(\Omega_j)} $. 
So, taking into account energy estimate (\ref{bee2})
we have for the first term
\begin{equation*} \label{eq:ss6}
\int_0^T S_1dt \leq C_{\mathcal{M}}^2 C_{\delta} P  \bigg( 1 + \frac{2C_{\mathcal{M}}^2P}{C_1\alpha} \bigg) h^2   \| \mathbf{V}_0^{\eps} \|_{H^1(\Omega)}^2 .
\end{equation*}
...
$$
Using one more time the energy estimate (\ref{bee2}) the second term in the right hand side of the above inequality  is bounded by $\frac{ C_{\mathcal{M}}^3 P h}{2 C_1 \widehat{\gamma}\eps} \| \V^\eps_0\|^2_{ L^2(\Omega)}$. Thus 
$$
\int _0^TS(t) dt \leq 
\frac{\widehat{\gamma} \sigma}{2\eps}\int_0^T\sum_r \sum_j \ljr \bigg( 
\njr  \: , \: \textbf{u}_r^{\eps}-\textbf{u}_j^{\eps} \bigg)^2 dt 
...
\begin{equation*}
\begin{aligned}
\mathscr{E}(T)& \leq C_0^2h^2 \|\mathbf{V}_0^{\eps}\|_{H^1(\Omega)}^2 \\
& 
-\frac1\eps \int_0^T \sum_{j,r}l_{j,r}(\textbf{n}_{j,r},\textbf{u}_j^{\eps}-\textbf{u}
_r^{\eps})^2\\
&  +  \frac{\gamma}{\eps}
...
_r^{\eps})^2 
+ T \frac {P C_{\delta}^2 C_{\mathcal{M}} h }{2\gamma \eps} \vert \vert \mathbf{V}_0^{\eps} \vert\vert^2_{H^1(\Omega)}  +  \bigg( \frac1{\alpha} + PC_{\delta}^2 \bigg) \frac{ h C_{\mathcal{M}}^2 }{2}\sqrt{\frac{P}{C_1}}  \| \mathbf{V}_0 \|_{H^1(\Omega)}^2   \\
& + \frac{hT}{\eps} C_{\mathcal{M}} C_{\delta} P \bigg( \frac{1+C_0^2h^2}{C_1} + h \bigg)   \|  \mathbf V ^\eps_0 \|^2_  { H^3(\Omega)  }  \\
&
+ \frac{\widehat{\gamma} \sigma}{2\eps}\int_0^T\sum_r \sum_j \ljr \bigg( 
\njr  \: , \: \textbf{u}_r^{\eps}-\textbf{u}_j^{\eps} \bigg)^2 dt 
+
...
\begin{equation*}
\begin{aligned}
C = \|\mathbf{V}_0^\eps\|^2_{H^3(\Omega)}  \: \hbox{max}\bigg\{ & \:  \frac{ C_{\mathcal{M}}^2 }{2}\sqrt{\frac{P}{C_1}} \bigg( \frac1{\alpha} + PC_{\delta}^2 \bigg) \: , \: \frac{C_{\mathcal{M}}^3P}{2C_1}  , \\
&  C_{\mathcal{M}} C_{\delta} P \bigg( \frac{1+C_0^2h^2}{C_1} +\frac{ C_{\delta} }{2 \gamma } + h \bigg) \: , \: C_0^2 + C_{\mathcal{M}}^2  P C_{\delta}\bigg( 1 + \frac{2C_{\mathcal{M}}^2P}{C_1\alpha} \bigg) \: \bigg\} .
\end{aligned}
\end{equation*}
...
$$
\mathscr{E}(T)\leq
Ch\left( 1 +\frac T \eps +h +\frac{\sigma}{\eps}
\right)$$
The constant $C$, uniform in $\eps$, is proportional to  $\|\V^\eps_0\|^2_{H^3(\Omega)} $, and  if $\sigma=0$ one recovers the result of Mazeran \cite{Mazeran}, chapter 4. The proof was more difficult to obtain due to the non standard discretization of the source term and its incorporation in  the approximate nodal Riemann solver. Since $h$ and $\eps$ can be taken less than 1, elementary comparison
principles 
yield
$\ds
\mathscr{E}(t) 
\leq  \frac{h}{\eps} C (2+T+\sigma) $.
The proof is ended. 
\end{proof}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
norm, in the sense that $E'(t)\leq 0$. Moreover, denoting $p_h=\left( p_j(0)\right)_{j\in Cells}$, we have
...
Then there exists a constant $C_c>0$ such that the following 
estimates hold
\begin{equation} \label{eq:d20}
\left| a_j\right|\leq C_c h
\quad
 \mbox{for all } j  ,
\end{equation}
\begin{equation} \label{eq:d21}
\left|  \br\right|  \leq C_c(h+\eps), \quad
\mbox{for all } r   .
\end{equation}
and
\begin{equation} \label{eq:d22}
\left| \cj\right| \leq C_c h, \quad
\mbox{for all } j   .
\end{equation}
\end{lemma}
...
\left\{
\begin{aligned}
&C_e(T) = \sqrt{2} e^{T/2} \max\bigg\{  \: C_0^2\|\mathbf{W}^{\eps}_0\|_{H^1(\Omega)}^2  \:  , \:  T\frac{2C_c^2|\Omega| }{\alpha \sigma} \max \bigg( 1 \: ,\: \frac{C_{\mathcal{M}}^2}{C_1^2\beta}\max\big(\alpha \sigma , C_{\mathcal{M}} P \big) \bigg)  \bigg\}^{\frac12}    ,\\
&C_f(T) = 2\sqrt{\frac{T+e^{-T}}{\alpha}}C_e , \\
&C_h(T) = \sqrt{2( T+e^{-T})}C_e , \\
&C_g(T) = \sqrt{\frac{2}{\beta} } C_{\mathcal{M}} \max\bigg( C_h(T) , \frac{C_{\mathcal{M}}P}{C_1}C_f^2(T) \bigg)^{\frac12} .
\end{aligned}
\right.
\end{equation*}
\end{theorem}
...
\end{aligned}
\end{equation*}
Using a Young's inequality, the Cauchy-Schwarz inequality and assumptions \eqref{eq:d5} and \eqref{geom3}, one gets
...
&
  -\eps\sum_j\sum_r \ljr \left( \fr- \fj,  \njr\right )^2 
+\frac{\eps}{2h  C_1}\left (\eta||\gf ||^2_{L^2(\Omega)}
...
we have
$$
\vert \Omega_j\vert \ \vert \fj \vert^2\leq \frac{C_{\mathcal{M}}^2}{\beta} h\sum_r\ljr(\njr,\fj)^2.
$$
Therefore
$$
\vert \Omega_j\vert \ \vert\fj \vert^2\leq 
\frac{C_{\mathcal{M}}^2}{\beta} h \left(2\sum_r\ljr(\njr,\fj-\fr)^2+2\sum_r \ljr \vert 
\fr \vert^2\right ),
$$
which yields, using \eqref{eq:d5.2} and \eqref{nbmr}
\begin{equation}\label{gl2}
\vert\vert\gf \vert\vert^2_{L^2(\Omega)} \leq 
\frac{2C_{\mathcal{M}}^2}{\beta} h\sum_{jr}\ljr(\njr,\fj-\fr)^2 +  \frac{2C_{\mathcal{M}}^3P}{\beta C_1} \vert\vert\ff \vert\vert^2_{L^2(\Omega)}.
\end{equation}
...
+\bigg( \frac{\mu}{2} + \frac{\eps\eta C_{\mathcal{M}}^3 P }{ h C_1^2 \beta} - \sigma \alpha \bigg) \|  \ff \|^2_{L^2(\Omega)}
\\
&+ \bigg(\frac{ C_{\mathcal{M}}^2 \eta}{C_1 \beta} -1 \bigg)\eps\sum_j\sum_r \ljr \left( \fr- \fj,  \njr\right )^2 
+\frac{\eps}{2h\eta C_1} ||\cc||^2, \qquad \forall \mu,\eta >0.
\end{aligned}
\end{equation*}
Let us choose $\mu=\frac{\alpha\sigma}{2}$ and $\eta =\frac{C_1\beta}{2C_{\mathcal{M}}^2} \min\big(1,\frac{\alpha \sigma hC_1}{ \eps C_{\mathcal{M}} P} \big)$. It yields, using the inequality $ab\leq \frac12(a^2+b^2)$
\begin{equation*} \label{eq:d40}
\begin{aligned}
E'(t) \leq & E(t) + \frac12  ||a||_{L^2(\Omega)}^2  - \frac{\alpha\sigma}{4}  F'(t)
- \frac{\eps}{2}\sum_j\sum_r \ljr \left( \fr- \fj,  \njr\right )^2\\
& +  \frac{1}{ \alpha\sigma } ||\bb||^2_{L^2(\Omega)}  +   \frac{C_{\mathcal{M}}^2}{C_1^2\beta} \max \bigg( \frac{\eps}{h} , \frac{\eps^2}{h^2} \frac{C_{\mathcal{M}} P}{ \alpha \sigma C_1 } \bigg) ||\cc||^2_{L^2(\Omega)} .
\end{aligned}
\end{equation*}
...
where
\begin{equation*}
C = \frac{2C_c^2|\Omega| }{\alpha \sigma} \max \bigg( 1 \: ,\: \frac{C_{\mathcal{M}}^2}{C_1^2\beta}\max\big(\alpha \sigma , C_{\mathcal{M}} P \big) \bigg).
\end{equation*}
Thus 
...
+  C\left (h+\eps\right )^2. 
\end{equation}
By construction $E(0)\leq C_0^2h^2\|\mathbf{W}^{\eps}_0\|_{H^1(\Omega)}^2$ \eqref{constant_init}.
 So by the Gr\"onwall lemma 
$
E(t)\leq C_e^2(t) (h+\eps)^2,
$
 with $C_e(t) = \sqrt{2} \max ( C_0^2\|\mathbf{W}^{\eps}_0\|_{H^1(\Omega)}^2 , T C )^{\frac12}e^{T/2}$,
...
\fr- \fj,  \njr\right )^2
\leq
 E(0)+ \int_0^t \left(  E(s)+ C(h+\eps)^2  \right) ds .
$$
Using the estimate \eqref{esti_E_DA}, it shows   
...
\frac{2C_{\mathcal{M}}^2}{\beta} h \int_0^T\sum_{jr}\ljr(\njr,\fj-\fr)^2 +  \frac{2C_{\mathcal{M}}^3P}{\beta C_1} \vert\vert\ff \vert\vert^2_{L^2([0,T]\times\Omega)}
\leq
C_g^2(T)(h+\eps)^2(1+\frac h\eps),
\end{equation*}
with
\begin{equation*}
C_g(T) = \sqrt{\frac{2}{\beta} } C_{\mathcal{M}} \max\bigg( C_h(T) , \frac{C_{\mathcal{M}}P}{C_1}C_f^2(T) \bigg)^{\frac12} . 
