...
For triangular mesh only the Boundary conditions based on th exact solutions are written **/
  if (d.Typemesh=='Q'){
    switch(d.nTest)
      {
      case 1 : BC_Dirichlet_SolExact_M1(d,Mh,v,Param,time);
    
	break;
	
      case 2 : BC_Periodic_M1(d,Mh,v,Param,time);
	
	break;
	
      case 3 : BC_Periodic_M1(d,Mh,v,Param,time);
	
	break;
	
      case 4 :BC_Periodic_M1(d,Mh,v,Param,time);
...
void BC_Dirichlet_SolExact_M1(Data & d,Mesh & Mh,variable & v, ParamPhysic & Param,double time){
  ...
  for(int j=0;j<Mh.nc;j++){
    if(Mh.cells[j].lab==-1){
	    ...
	    if((Mh.xj(j).x < 0) && (Mh.xj(j).y> 0 && Mh.xj(j).y< d.Ty)){
	      v.var[0][j]=SolFonM1(d,Param,Mh.xj(j),time,Mh.xj(in),0);
	      v.var[1][j]=SolFonM1(d,Param,Mh.xj(j),time,Mh.xj(in),1);
              v.var[2][j]=SolFonM1(d,Param,Mh.xj(j),time,Mh.xj(in),2);
