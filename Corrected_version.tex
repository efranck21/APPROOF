...
$\V^{\eps}_h(t)=
\left( \sum_{j\in \mathbb Z} p_j^{\eps}(t) {1}_{\Omega_j} ,  \sum_{j\in \mathbb Z} u_j^{\eps}(t) {1}_{\Omega_j} \right)$ the solution of the JL-(b) scheme $P_h^\eps$. Similarly  we  reconstruct
...
$P^0$ (\ref{difffff})-(\ref{eq:diff-v}),
 and  $\W_h^\eps(t)= \left( \sum_{j\in \mathbb Z} p_j(t) {1}_{\Omega_j} ,
\varepsilon \sum_{j\in \mathbb Z} v_j(t)  {1}_{\Omega_j} \right)$ which is
the
...
\begin{itemize}
\item $\mathbf V_h^\eps=\left(  \sum_{j\in Cells} p^\eps_j 1_{\Omega_j} , \sum_{j\in Cells} \uu^\eps_j  1_{\Omega_j} \right) $ is the solution of $\mathbf P_h^\eps$.
\item 
$\mathbf V^\eps=\left(p^\eps, \uu^\eps  \right)$ is the solution of $\mathbf P^\eps$,
\item $\mathbf W_h^\eps=\left(  \sum_{j\in Cells}  p^\eps_j 1_{\Omega_j} ,  \sum_{j\in Cells}  \uu^\eps_j  1_{\Omega_j}  \right) $ is the solution of $\mathbf{DA}_h^\eps$.
Notice that an abuse of notations is been made with the solution
of $\mathbf P_h^\eps$.
\item $\mathbf W^\eps= \left( p ,-\frac\eps\sigma \nabla p  \right) $ is the solution of $\mathbf P^0$.
\end{itemize}
